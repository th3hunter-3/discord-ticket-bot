 ...
 * Generates HTML and PDF transcripts of ticket conversations
 */
...
/**
 * Generate PDF transcript from HTML
 * @param {string} html - HTML content
 * @param {string} ticketId - Ticket ID
...
export async function generatePDFTranscript(html, ticketId) {
  try {
    ...
    logger.warn('Falling back to HTML transcript only');
    return await saveTranscript(html, ticketId);
  }
}
